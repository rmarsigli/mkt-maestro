...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full print:px-0 print:py-0 print:max-w-none">
	...
	<div class="flex items-start gap-3 mb-6 print:mb-8">
		<div class="w-10 h-10 rounded-xl bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center flex-shrink-0 print:hidden">
			<FileText class="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
		</div>
		<div>
			<h1 class="text-xl font-bold text-slate-900 dark:text-white leading-snug print:text-2xl">{data.slug}</h1>
			{#if data.date}
	...
	<article class="prose prose-slate dark:prose-invert max-w-none
		...
		prose-code:bg-slate-100 dark:prose-code:bg-slate-800 prose-code:rounded prose-code:px-1 prose-code:py-0.5 prose-code:text-[0.85em]
